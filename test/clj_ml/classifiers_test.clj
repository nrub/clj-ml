(ns clj-ml-dev.classifiers-test
  (:use [clj-ml-dev classifiers data] :reload-all)
...
(deftest train-classifier-c45
  (let [c (make-classifier :decision-tree :c45)
        ds (clj-ml-dev.data/make-dataset "test" [:a :b {:c [:m :n]}] [[1 2 :m] [4 5 :m]])]
    (clj-ml-dev.data/dataset-set-class ds 2)
    (classifier-train c ds)
    (is true)))

(deftest make-classifier-bayes
  (fact
    (let [c (clj-ml-dev.classifiers/make-classifier :bayes :naive {:kernel-estimator true :old-format true})
          opts (vec (.getOptions c))]
      opts => (contains ["-K" "-O"]))))

(deftest make-classifier-bayes-updateable
  (let [c (clj-ml-dev.classifiers/make-classifier :bayes :naive {:updateable true})]
    (is (= (class c)
...
(deftest classifier-evaluate-dataset
  (let [c   (make-classifier :decision-tree :c45)
        ds  (clj-ml-dev.data/make-dataset "test" [:a :b {:c [:m :n]}] [[1 2 :m] [4 5 :m]])
        tds (clj-ml-dev.data/make-dataset "test" [:a :b {:c [:m :n]}] [[4 1 :n] [4 5 :m]])
        _   (clj-ml-dev.data/dataset-set-class ds 2)
        _   (clj-ml-dev.data/dataset-set-class tds 2)
